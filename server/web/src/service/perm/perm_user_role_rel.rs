//! 用户角色关联关系管理
use crate::{
    dao::perm::perm_user_role_rel::PermUserRoleRelDao,
...
/// 服务
#[injectable]
pub struct PermUserRoleRelService<'a> {
    user_role_rel_dao: PermUserRoleRelDao<'a>,
}

impl<'a> PermUserRoleRelService<'a> {
    /// 获取列表数据
    pub async fn list(
        &self,
