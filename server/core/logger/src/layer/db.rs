...
impl FlushDb {
    ...
    /// 数据入库
    async fn save_db(&self, output: Model) {
...
/// 输出到数据库中
/// "v":0,"name":"console_bunyan_layer","msg":"[INNER - END]", "hostname":"one","pid":60201,
    ...
    for<'a> S: LookupSpan<'a>,
{
    ...
    let layer = JsonLayer::new(config.log_name.clone()).with_max_level(config.level.clone().into());
    Box::new(layer)
}
